<div id="new-custo-fixo-modal" class="board-modal fixed inset-0 z-50 hidden bg-black/60 backdrop-blur-sm" aria-hidden="true">
    <div class="flex min-h-full items-center justify-center px-4 py-8">
        <div class="modal-panel w-full max-w-md rounded-2xl border border-white/10 bg-slate-900/95 p-6 shadow-2xl shadow-black/40">
            <div class="mb-5 flex items-start justify-between gap-4">
                <h2 class="text-lg font-semibold text-white">Novo custo fixo</h2>
                <button
                    type="button"
                    data-modal-close
            ...
            <form id="new-custo-fixo-form" action="{{ route('financeiro.custos-fixos.store', absolute: false) }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label for="custo-fixo-name" class="mb-1.5 block text-sm font-medium text-slate-300">Nome</label>
                    <input
                        id="custo-fixo-name"
                        type="text"
                        name="name"
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label for="custo-fixo-category" class="mb-1.5 block text-sm font-medium text-slate-300">Categoria</label>
                        <select
                            id="custo-fixo-category"
                            name="category"
                            required
                    ...
                    <div>
                        <label for="custo-fixo-amount" class="mb-1.5 block text-sm font-medium text-slate-300">Valor</label>
                        <input
                            id="custo-fixo-amount"
                            type="number"
                            step="0.01"
                            min="0.01"
                            name="amount"
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label for="custo-fixo-due-day" class="mb-1.5 block text-sm font-medium text-slate-300">Dia de vencimento</label>
                        <input
                            id="custo-fixo-due-day"
                            type="number"
                            name="due_day"
                            min="1"
                            max="31"
                    ...
                    <div>
                        <label for="custo-fixo-periodicity" class="mb-1.5 block text-sm font-medium text-slate-300">Periodicidade</label>
                        <select
                            id="custo-fixo-periodicity"
                            name="periodicity"
                            required
                            class="w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2.5 text-sm text-white outline-none transition focus:border-indigo-400/50 focus:ring-2 focus:ring-indigo-400/20"
                        >
                            @foreach (\App\Models\CustoFixo::PERIODICITIES as $periodicity)
                                <option value="{{ $periodicity }}" @selected($periodicity === 'mensal')>{{ ucfirst($periodicity) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label for="custo-fixo-starts-on" class="mb-1.5 block text-sm font-medium text-slate-300">Início</label>
                        <input
                            id="custo-fixo-starts-on"
                            type="date"
                            name="starts_on"
                            required
                    ...
                    <div>
                        <label for="custo-fixo-ends-on" class="mb-1.5 block text-sm font-medium text-slate-300">Fim (opcional)</label>
                        <input
                            id="custo-fixo-ends-on"
                            type="date"
                            name="ends_on"
                            class="w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2.5 text-sm text-white outline-none transition focus:border-indigo-400/50 focus:ring-2 focus:ring-indigo-400/20 scheme-dark"
                        >
                ...
                <div>
                    <label for="custo-fixo-notes" class="mb-1.5 block text-sm font-medium text-slate-300">Observação</label>
                    <textarea
                        id="custo-fixo-notes"
                        name="notes"
                        rows="2"
